...
async function confirmEnableOrDisable() {
  const result = await ElMessageBox.confirm('确定改变该用户状态吗?', '警告', {
...
<template>
  <div>
    <div mb-4 flex justify-between>
      <div>
        <el-button type="primary" round @click="addUserDialog = true">
          新增用户
        </el-button>
      </div>
      <div>
        <el-button round @click="handleGetUserList">
          刷新列表
        </el-button>
      </div>
    </div>
    ...
    <el-table v-loading="loading" :data="list" stripe border width="100%" row-key="uuid">
      ...
      <el-table-column label="身份" prop="isAdmin" width="120" align="center">
        <template #default="{ row }">
          <el-tag v-if="row.isAdmin === '1'" type="warning">
            管理员
          </el-tag>
      ...
      <el-table-column label="操作" width="200" align="center">
        <template #default="{ row }">
          <el-button type="warning" text link @click="handleUpdateUser(row.id)">
            编辑
          </el-button>
